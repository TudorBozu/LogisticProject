...
export default function VehicleInfoCard({ truck }: Props) {
    const { setActiveTab } = useFleet();
    const { lang } = useLang();
    const t = fleetT[lang];

    const cap = Math.max(0, Math.min(100, truck.usedCapacityPct));
    const fuel = Math.max(0, Math.min(100, truck.fuelPct));
    const alertCount = truck.alertCount ?? 0;

    function handleCall() {
        if (truck.driver.phone) {
            window.open(`tel:${truck.driver.phone}`);
        }
    }
        ...
        <div className="bg-white rounded-[28px] shadow-soft border border-slate-100 overflow-hidden">
            <div className="p-5">
                ...
                <div className="flex items-center justify-between">
                    <div className="text-[12px] text-slate-400">{t.vehicle}</div>
                    ...
                    <div className="flex items-center gap-3">
                        <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-100 text-[12px] font-semibold text-slate-700">
                            <span className={`w-2 h-2 rounded-full ${statusDot[truck.status]}`} />
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                    <div>
                        <div className="text-[12px] text-slate-400">{t.burden}</div>
                    ...
                    <div className="relative pl-4">
                        ...
                        <div className="text-[12px] text-slate-400">{t.usedVolume}</div>
                        <div className="mt-1 text-[12.5px] font-semibold text-slate-700">
                ...
                <div className="mt-4 grid grid-cols-[1fr,120px,42px] items-center gap-3">
                    <div className="text-[12px] text-slate-400">{t.usedCapacity}</div>
                ...
                <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="leading-tight">
                            <div className="text-[11px] text-slate-400">{t.driver}</div>
                            <div className="text-[12.5px] font-semibold text-slate-800">{truck.driver.name}</div>
                            {truck.driver.phone && (
                                <div className="text-[10px] text-slate-400">{truck.driver.phone}</div>
                            )}
                        </div>
                    </div>
                ...
                <div className="mt-4 rounded-[22px] bg-gradient-to-b from-white to-blue-50 border border-slate-100 p-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="rounded-[18px] bg-white/70 border border-white/70 p-4">
                            <div className="text-[12px] text-slate-400">{t.fuel}</div>
                        ...
                        {/* Temperature */}
                        <div className="rounded-[18px] bg-white/70 border border-white/70 p-4">
                            <div className="text-[12px] text-slate-400">{t.temperature}</div>
                            <div className="mt-2 text-[22px] font-extrabold text-slate-900">
                                {truck.temperatureC}°C
                            </div>

                            <div className="mt-3 h-[62px] rounded-[16px] bg-gradient-to-b from-[#00B7FF] to-[#5393ED] border border-white/40 grid place-items-center text-[12px] text-white font-medium" />
                        </div>
                    </div>
                </div>
            </div>

            {/* Map / Alerts action row */}
            <div className="px-5 pb-5">
                <div className="rounded-[22px] bg-slate-50 border border-slate-100 p-2 flex gap-2">
                    <button
                        type="button"
                        className="flex-1 h-12 rounded-[18px] bg-white border border-slate-100 text-[14px] font-semibold text-slate-800 hover:bg-slate-50 transition flex items-center justify-center gap-2"
                    ...
                    <button
                        type="button"
                        className="flex-1 h-12 rounded-[18px] bg-white border border-slate-100 text-[14px] font-semibold text-slate-800 hover:bg-slate-50 transition flex items-center justify-center gap-2"
                        onClick={() => alertCount > 0 && alert(`${alertCount} alert(s) for ${truck.code}`)}
                    >
