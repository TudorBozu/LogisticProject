...
function validate(values: SignInValues, errors: {
  ...
  if (!values.email) errs.push({ field: 'email', message: errors.emailRequired })
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.email)) errs.push({ field: 'email', message: errors.emailInvalid })
  if (!values.password) errs.push({ field: 'password', message: errors.passwordRequired })
  else if (values.password.length < 6) errs.push({ field: 'password', message: errors.passwordTooShort })
  return errs
}
...
export default function SignInPage() {
  ...
  const panelStats = t.panelStats.map((s, i) => ({
    ...
    label: s.label,
    icon: [
      <svg key={i} width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>,
      ...
      <svg key={i} width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941"/></svg>,
    ][i],
  }))
